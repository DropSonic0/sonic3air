...
Json::Value OxygenJsonHelper::loadFile(const std::wstring& filename)
{
...
bool OxygenJsonHelper::saveFile(const std::wstring& filename, const Json::Value& value)
{
	const String output(value.toStyledString());
	return FTX::FileSystem->saveFile(filename, *output, output.length());
}

bool OxygenJsonHelper::parseWString(std::wstring& output, const Json::Value::const_iterator& it)
{
...
bool OxygenJsonHelper::parseVec2i(Vec2i& output, const Json::Value::const_iterator& it)
{
	if (it->isString() && !it->asString().empty())
	{
...
bool OxygenJsonHelper::parseRecti(Recti& output, const Json::Value::const_iterator& it)
{
	if (it->isString() && !it->asString().empty())
	{
		String(it->asString()).split(tempParts, ',');
